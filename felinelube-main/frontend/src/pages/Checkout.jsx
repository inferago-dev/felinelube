  ...
  if (cartItems.length === 0 && !success) {
    return (
      <div style={{ paddingTop: '100px', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <HiOutlineShoppingCart style={{ fontSize: '4rem', color: 'var(--color-text-dim)', marginBottom: '1rem' }} />
    ...
    return (
      <div style={{ paddingTop: '100px', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 200, damping: 20 }}>
          <HiOutlineCheckCircle style={{ fontSize: '6rem', color: 'var(--color-gold)', marginBottom: '1rem' }} />
        </motion.div>
        <h2 style={{ marginBottom: '1rem' }}>Order Placed Successfully!</h2>
        <p style={{ color: 'var(--color-text-dim)', maxWidth: '500px', marginBottom: '2rem' }}>
    ...
    <div className="auth-page">
      <div style={{ width: '100%', maxWidth: '1000px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', padding: '2rem 1rem' }}>
        ...
        <motion.div 
          ...
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          style={{ width: '100%', background: 'var(--color-surface-card)' }}
        >
          <div className="auth-header" style={{ textAlign: 'left', borderBottom: '1px solid var(--color-border)', paddingBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.25rem' }}>Order Summary</h2>
          </div>
          
          <div style={{ padding: '1rem 0', display: 'flex', flexDirection: 'column', gap: '1rem', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <div style={{ paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.25rem', fontWeight: 'bold', marginTop: '0.5rem', color: 'var(--color-text)' }}>
              <span>Total</span>
